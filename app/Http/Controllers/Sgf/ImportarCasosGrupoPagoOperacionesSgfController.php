...
class ImportarCasosGrupoPagoOperacionesSgfController extends Controller
{
    ...
    public function store(): RedirectResponse
    {
        ...
        $trabajoEnCurso = TrabajoIntegracion::where('sistema_externo_id', $sistema->id)
            ->where('tipo', 'importar_grupo_pago_operaciones')
        ...
        if ($trabajoEnCurso !== null && $trabajoEnCurso->estado === 'en_progreso') {
            Inertia::flash('toast', ['type' => 'info', 'message' => 'Ya hay una importación del grupo Pago Operaciones de SGF en curso.']);

            return to_route('sgf.importaciones.show', $trabajoEnCurso);
        }

        $trabajo = $this->integracionExterna->iniciarTrabajo($sistema, 'importar_grupo_pago_operaciones', 'playwright');

        ImportarCasosGrupoPagoOperacionesSgfJob::dispatch($trabajo);
