...
Esta capacidad cubre la gestión de `Contrato` como entidad de dominio propia con workflow propio (`borrador → pendiente → aprobado`/`rechazado`, gobernado exclusivamente por `TransicionWorkflowService::execute()`), sus sub-entidades de ítems de convenio de precio y calendario de pago (cuotas), y sus vínculos opcionales e informativos con Adquisiciones (`ProcesoAdquisicion`, `LicitacionMercadoPublico`, `OrdenCompraMercadoPublico`) y con Pago de Proveedores (`CasoPagoProveedor`), siguiendo el mismo patrón entidad+workflow ya validado en Adquisiciones y en el Certificado de Disponibilidad Presupuestaria.
