...
- **Permisos**: convención `modulo_accion.verbo` en snake_case en ambos lados (p. ej. `pago_proveedores.registrar_cgu`, `usuarios.resetear_password`, `core_institucional.administrar`) — así están definidos en todos los seeders reales (`RolesAndPermissionsSeeder`, `Workflow*Seeder`). Usar siempre este formato para permisos nuevos.
