...
### Disciplina de controladores livianos en OpenSpec (propose/apply)

"Controladores livianos" (arriba) no basta como directriz pasiva a esperar que una auditoría posterior detecte — debe exigirse activamente durante el ciclo OpenSpec, no después. Lección de una auditoría real (2026-07, ver `openspec/changes/archive/2026-07-16-extraer-logica-negocio-controllers-pago-proveedores`): varios controllers de Pago de Proveedores terminaron con queries de negocio (listas de códigos de estado hardcodeadas + `whereHas`), una `DB::transaction` completa, decisiones de 3 ramas, y `app(Service::class)` como service locator dentro de un `map()` — porque al menos un `tasks.md` ya aprobado dejó la ubicación de la lógica abierta ("en el propio controlador o en un método estático... según lo que quede más legible"). Corregirlo después obligó a reabrir y enmendar dos changes ya marcados `[x]` en todas sus tareas.

- Al escribir `tasks.md` en `/opsx:propose`: nunca dejar la ubicación de la lógica como opción abierta entre "controlador o service". Si el método hace más que un `find`/`paginate`/`create` 1:1 desde el request, la tarea nombra explícitamente el Service y su método.
- Al ejecutar `/opsx:apply`: antes de marcar una tarea como hecha, revisar el método de controller recién escrito. Si tiene `whereHas`/`where` anidados expresando una regla de negocio, un `DB::transaction`, 2+ ramas condicionales de negocio, o un `app(Clase::class)` — extraerlo a un Service en `app/Services/<Dominio>/` antes de continuar.
- Reutilizar las convenciones de nombrado ya establecidas por dominio: sufijo `Service` (orquestación multi-operación), `Presenter` (payload de una pantalla), `Resolver` (una pregunta booleana/de negocio), `Importer`/`Creador` (creación transaccional).
- Antes de `/opsx:archive`: revisar rápidamente los controllers tocados contra esta regla — que los tests pasen no confirma que la lógica esté en el lugar correcto.
