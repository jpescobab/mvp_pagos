...
import type { CdpFormValues } from '@/components/presupuesto/cdp-form';
import cdps from '@/routes/presupuesto/cdps';
import type {
    CertificadoDisponibilidadPresupuestaria,
...
export default function CdpEdit() {
    const { cdp, lineasPresupuesto, procesosAdquisicion } =
        usePage<PageProps>().props;

    const [valores, setValores] = useState<CdpFormValues>({
        presupuesto_id: String(cdp.presupuesto_id),
        fecha_solicitud: cdp.fecha_solicitud ?? '',
        tipo_gasto: cdp.tipo_gasto,
        codigo_iniciativa: cdp.codigo_iniciativa ?? '',
        nombre: cdp.nombre,
        nombre_iniciativa: cdp.nombre_iniciativa ?? '',
        caracter_gasto: cdp.caracter_gasto,
        medio_solicitud: cdp.medio_solicitud ?? SIN_MEDIO_SOLICITUD,
        requerimiento_numero: cdp.requerimiento_numero ?? '',
        moneda_compra: cdp.moneda_compra,
        total_moneda_compra: cdp.total_moneda_compra,
        fecha_paridad: cdp.fecha_paridad ?? '',
        anio_validez: String(cdp.anio_validez),
        proceso_adquisicion_id: cdp.proceso_adquisicion
            ? String(cdp.proceso_adquisicion.id)
            : SIN_ADQUISICION,
    });
